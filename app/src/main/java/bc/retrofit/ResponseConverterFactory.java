...
public class ResponseConverterFactory extends Converter.Factory {
    ...
    private class BaseResponseBodyConverter<T> implements Converter<ResponseBody, T> {
        ...
        public T convert(ResponseBody response) throws IOException {
            ...
            HttpResponse<T> result = mGson.fromJson(strResponse, HttpResponse.class);
            if (result.getState() != 1) {
                throw new HttpException(result.getMsg());
            }
            if (result.getRes().getCode() == 40000) {
                String resultString = result.getRes().getData().toString();
                if (TextUtils.isEmpty(resultString)) {
                    throw new HttpException("请求服务器异常");
                }
                return mGson.fromJson(mGson.toJson(result.getRes().getData()), mType);
            } else {
                throw new HttpException(result.getRes().getMsg());
            }
//            try {
