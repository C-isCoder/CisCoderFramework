...
public class InteractionContentPresentImpl
        ...
        recyclerView.addOnScrollListener(new OnScrollListener() {
            @Override public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                if (RecyclerViewUtils.isScrollBottom(recyclerView)
                    ...
                    nowPage++;
                    if (typeId != -1) {
                        mInteraction.getInteractionList(typeId, nowPage,
                        ...
                        switch (modelType) {
                            ...
                            case InteractionInfoPresent.DYNAMIC:
                                ...
                                break;
                            case InteractionInfoPresent.COLLECT: // 收藏的动态
                                mInteraction.getCollect(nowPage,
                                    InteractionContentPresentImpl.this);
                                break;
                        }
                    }
    ...
    @Override public void praise(final InteractionListData data) {
        // 点赞
        mInteraction.praise(data.id, new BaseListener<Boolean>() {
            @Override public void success(Boolean aBoolean) {
                mAdapter.updatePraise(data.id, aBoolean);
            }

            @Override public void error(String error) {
                mAdapter.updatePraise(data.id, false);
            }
        });
    }
    ...
    private BaseListener<Boolean> cancelCollectListener = new BaseListener<Boolean>() {
        @Override public void success(Boolean aBoolean) {
            if (aBoolean) {
                mView.showMsg("取消成功");
                EventBus.getDefault()
                    .post(new BaseEventData(Event.INTERACTION_LIST_CANCEL_COLLECT));
            } else {
                mView.showMsg("取消失败");
            }
        }
